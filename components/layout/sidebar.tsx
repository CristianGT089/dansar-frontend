...
export function Sidebar() {
  ...
  const filtered = dashboardNavItems.filter((item) => !item.superadminOnly || user?.is_superadmin);

  // Determina el título de la sección activa para el top bar móvil
  const activeItem = dashboardNavItems.find((item) => pathname.startsWith(item.href));
  ...
  return (
    <>
      ...
      <MobileNav
        ...
        title={activeItem?.label ?? "Dan-Sar"}
        subtitle="Contable"
        logoInitials="DS"
      />
          ...
          {filtered.map((item) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "group flex items-center gap-3 rounded-md px-2 py-2.5 text-sm font-medium transition-colors",
